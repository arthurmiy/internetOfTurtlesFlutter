...
class _InfoPulgaState extends State<InfoPulga> {
  //opções de atualiação de tela
  static const String opt1Manual = 'Manualmente';
  static const String opt2fastest = 'O mais rápido possível';
  static const String opt3each15s = 'A cada 15 segundos';
  static const String opt4each1m = 'A cada minuto';
  static const String opt5each5m = 'A cada 5 minutos';
                        ...
                        IntrinsicWidth(
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              SensorDataIcon(
                                  Icons.wb_sunny, '$uvLevel', 'Nível UV'),
                              Padding(
                                ...
                                child: OutlineButton(
                                  onPressed: () {
                                    setState(() {
                                      ledOn = !ledOn;
                                      deviceData.services[2].characteristics[3]
                                          .characteristic
...
//                                                arguments: deviceData);
  ...
  void refreshChar1(List<int> value) {
    int mode = value[0] - 1;
    if (mode == 0) {
      temperature = value[1] * 256 * 256 + value[2] * 256 + value[3];
      print(temperature);
    } else if (mode == 1) {
      brightness = value[1] * 256 * 256 + value[2] * 256 + value[3];
    } else if (mode == 2) {
      ...
      //tensão
      voltage = value[1] * 256 * 256 + value[2] * 256 + value[3];
      //voltage = voltage * 10;
      print('$voltage');
    }
    //setState(() {});
  ...
  void refreshChar2(List<int> value) {
    int mode = value[0] - 1;
    if (mode == 0) {
      pressure = value[1] * 256 * 256 + value[2] * 256 + value[3];
    } else if (mode == 1) {
      uvLevel = value[1] * 256 * 256 + value[2] * 256 + value[3];
    } else if (mode == 2) {
      int val = value[1] * 256 * 256 + value[2] * 256 + value[3];
      FlSpot tmp = getNewData(
          (val <= 32767)
  ...
  void refreshChar3(List<int> value) {
    int mode = value[0] - 1;
    if (mode == 0) {
      moisture = value[1] * 256 * 256 + value[2] * 256 + value[3];
